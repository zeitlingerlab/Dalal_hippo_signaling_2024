---
title: 'Curation of motifs from TSC model'
author: "Khayti Dalal"
...
The goal of this analysis is to curate motifs found by the `TSC model` TF-MoDISco run such that there are no redundancies due palindromes, sequence similarity, overlapping peak regions etc. 
...
tasks <- c('cdx2','tead4','tfap2c','gata3','yap1')
threads <- 5
...
motifs_of_interest.df <- data.frame(
  name = c('tfap2c_0','tfap2c_1','tead4_0','cdx2_0','gata3_1','gata3_4','gata3_5','tead4_double_1','ctcf','elf5','jun_fos','yap1_0','yap1_1'), 
  task = c('tfap2c','tfap2c','tead4','cdx2','gata3','gata3','gata3','tead4','cdx2','yap1','cdx2','yap1','yap1'),
  pattern = c(0,1,0,0,1,4,5,1,2,6,4,0,1)
)
motifs_of_interest.df

# motif set of interest for fold 3
# motifs_of_interest.df <- data.frame(
#   name = c('tfap2c_0','tfap2c_3','tead4_0','cdx2_0','cdx2_1','gata3_1','gata3_2','gata3_4','tead4_double_1','ctcf','elf5','jun_fos','yap1_0','yap1_3'), 
#   task = c('tfap2c','tfap2c','tead4','cdx2','cdx2','gata3','gata3','gata3','tead4','tead4','yap1','yap1','yap1','yap1'),
#   pattern = c(0,3,0,0,1,1,2,4,1,2,4,7,0,3)
# )
# motifs_of_interest.df

# motif set of interest for fold 4
# motifs_of_interest.df <- data.frame(
#   name = c('tfap2c_0','tfap2c_4','tead4_0','cdx2_0','gata3_1','gata3_2','tead4_double_1','ctcf','elf5','jun_fos','yap1_0','yap1_4'), 
#   task = c('tfap2c','tfap2c','tead4','cdx2','gata3','gata3','tead4','tead4','yap1','yap1','yap1','yap1'),
#   pattern = c(0,4,0,0,1,2,1,2,3,6,0,4)
# )
# motifs_of_interest.df
```
...
cmds.vec<-lapply(1:nrow(motifs_of_interest.df), function(x){
  ...
  cmd<-paste0('bpnet cwm-scan --patterns metacluster_0/pattern_', info.df$pattern, ' ',' ',
         modisco_dir, '/', info.df$task,' ',' ', '/n/projects/kd2200/analysis/BPNet/cegkttyz_tsc_bpnet/grid_search/final_cttgy_tsc_model/bpnet/analysis/fold_5/tsv/',
...
readr::write_lines(cmds.vec, file = '/n/projects/kd2200/publication/bpnet/analysis/tmp/tsc_model_cwm_scan_indiv_instances.sh', sep = '\n')
```
...
<!-- bash /n/projects/kd2200/publication/bpnet/analysis/tmp/tsc_model_cwm_scan_indiv_instances.sh -->
```
...
motifs.df<-lapply(1:nrow(motifs_of_interest.df), function(x){
    info.df<-motifs_of_interest.df[x,]
    df<-readr::read_tsv(paste0('/n/projects/kd2200/publication/bpnet/analysis/tsv/motif_curation/', info.df$name, '_instances-all-regions.tsv.gz')) %>%
...
ggplot(motifs.gr %>% as.data.frame, aes(x = pattern_name))+
    ...
    ggtitle('TSC curated motif frequencies')+
    theme_classic()
```
...
rtracklayer::export(islands.gr %>% plyranges::mutate(name = region_id), 'bed/curated_island_regoion_tsc_model_motifs_fold5_0based.bed', format = 'BED') 
```
...
readr::write_tsv(motifs_curated.df, 'tsv/curated_motif_regoion_tsc_model_motifs_fold5_1based.tsv.gz')
rtracklayer::export(motifs_curated.gr %>% plyranges::mutate(name = paste0(pattern_name, '_', region_id)), 
                    'bed/curated_motif_regoion_tsc_model_motifs_fold5_0based.bed', format = 'BED')
...
readr::write_tsv(motifs_curated2.df, 'tsv/curated_motif_regoion_tsc_model_motifs_fold5_1based_noerv_nopromoter.tsv.gz')
rtracklayer::export(gr_no_erv_no_promo %>% plyranges::mutate(name = paste0(pattern_name, '_', region_id)), 
                    'bed/curated_motif_regoion_tsc_model_motifs_fold5_0based_noerv_nopromoter.bed', format = 'BED')

##noerv
motifs_curated3.df<-gr_no_erv %>% as.data.frame()
readr::write_tsv(motifs_curated3.df, 'tsv/curated_motif_regoion_tsc_model_motifs_fold5_1based_noerv.tsv.gz')
rtracklayer::export(gr_no_erv %>% plyranges::mutate(name = paste0(pattern_name, '_', region_id)), 
                    'bed/curated_motif_regoion_tsc_model_motifs_fold5_0based_noerv.bed', format = 'BED')
