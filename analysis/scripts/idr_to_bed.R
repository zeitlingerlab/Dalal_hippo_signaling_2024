...
option_list <- list(
  make_option(c("-i", "--idr"),
              type="character",
              default=NA,
              help="Path of idr.txt file to process"),
...
colnames(regions.df)<-c("chr", "start", "end", "string", "idr_score",
...
export(regions_standard.gr, opt$output, format="BED")
