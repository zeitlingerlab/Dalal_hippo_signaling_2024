...
mark_nonredundant_bpnet_windows<-function(instances, window_length = 1000){
  ...
  example_idx_filt.df <- instances.gr %>% as.data.frame %>%
    ...
    slice_max(order_by = motif_count, n = 1, with_ties = FALSE) %>%  #get top motif counts
    slice_max(order_by = min(central_score), with_ties = FALSE) #get top by most centered motifs
