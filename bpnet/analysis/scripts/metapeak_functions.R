...
# normalize matrix for standard_matrix

normalize_single_matrix<-function(matrix, removal_threshold, normalize_threshold){

  max.per.gene <- apply(matrix, 1, function(x){quantile(x, normalize_threshold)})
  min.per.gene <- apply(matrix, 1, function(x){quantile(x, removal_threshold)})


  matrix [matrix <= min.per.gene] <- NA #Remove all values that are below 50th percentile
  matrix <- pmin(matrix / max.per.gene,1) #Re-normalize values that remain.
  matrix
}
...
# plot_nexus_heatmap<-function(regions.gr, sample, title="Heatmap of ChIP-nexus Signals", order="sum", output_file_name=NA, reduce=F, upstream=50, downstream=50,
...
#   mat<-exo_metapeak_matrix(regions.gr = regions.gr, sample=sample, upstream=upstream, downstream=downstream)
# 
#   #Order region rows
#   if(order=="clustering"){
...
#     #Plot matrix
#     g<-ggplot()+
#       geom_raster(data=mat_pos.df, aes(x=position, y=region, fill=signal))+
#       geom_raster(data=mat_neg.df, aes(x=position, y=region, fill=-signal), alpha=alpha_value)+
...
#       theme_classic()+
#       theme(text=element_text(size=14), legend.position = "bottom", panel.grid = element_blank(), panel.border = element_blank())
# 
...
plot_nexus_heatmap<-function(regions.gr, sample, title="Heatmap of ChIP-nexus Signals", order="sum", output_file_name=NA, reduce=F, upstream=50, downstream=50,
                             removal_threshold=.5, normalize_threshold=0.99, reverse=F, return_only_df = F, ncol = 2,text_size = 14, alpha_value = 1, neg_color ="red",
                             pos_color = "royalblue"){
  ...
  if(return_only_df){
    ...
    #Plot matrix
    g<- ggplot()+
      geom_raster(data= mat_pos.df, aes(x=position, y=region, fill=signal), alpha=alpha_value)+
      geom_raster(data=mat_neg.df, aes(x=position, y=region, fill=-signal), alpha=alpha_value)+
      ggtitle(title)+
      scale_x_continuous("Position (bp)", expand = expand_scale(mult = c(0, 0)))+
      scale_y_continuous("Regions", expand = expand_scale(mult = c(0, 0)))+
      scale_fill_gradient2(low=neg_color, mid="white", high=pos_color)+
      theme_classic()+
      theme(panel.background = element_rect(fill = "transparent", colour = NA),  plot.background = element_rect(fill = "transparent", colour = NA),
            text=element_text(size= text_size), legend.position = "bottom", legend.key.size = unit(0.3, "cm"), panel.grid = element_blank(), panel.border = element_blank())
...
plot_standard_heatmap<-function(regions.gr,sample, title="Heatmap of ChIP-seq Signals", order="sum", output_file_name=NA, reduce=F, upstream=50, downstream=50,alpha_value = 1,
                                removal_threshold=.5, normalize_threshold=0.99, reverse=F, return_only_df = F, ncol = 2, text_size = 14, low_color= "white", high_color= "#67000d"){
  ...
  mat <- standard_metapeak_matrix(regions.gr, sample, upstream = upstream,
  ...
  if(order=="clustering"){
    ...
    order_of_rows<-1:length(regions.gr)
  }
  
  mat<-mat[order_of_rows, ]
  #Normalize matrix
  norm_mat_matrix<-normalize_single_matrix(matrix= mat, removal_threshold =  removal_threshold, normalize_threshold = normalize_threshold)
  #Format matrix
  mat_final_df<-format_matrix_for_heatmap(matrix = norm_mat_matrix, strand = "*", downstream = downstream, upstream = upstream)
  ...
  if(return_only_df){
    ...
    #Plot matrix
    g <- ggplot()+
      geom_raster(data=mat_final_df, aes(x=position, y=region, fill=signal), alpha = alpha_value)+
      ggtitle(title)+
      scale_x_continuous("Position (bp)", expand = expand_scale(mult = c(0, 0)))+
      scale_y_continuous("Regions", expand = expand_scale(mult = c(0, 0)))+
...
##################################################################################################################################################################################################################################################################################################################
plot_multiple_gr_nexus_heatmap<-function(gr.list, sample, title="Heatmap of ChIP-nexus Signals", order="sum", output_file_name=NA, reduce=F, upstream=50, downstream=50,
                                removal_threshold=.5, normalize_threshold=0.99, reverse=F, return_only_df = F, ncol = 2,text_size = 14, alpha_value = 1, neg_color ="red",
                                pos_color = "royalblue"){
  mat <- mclapply(names(gr.list), function(x){
    regions.gr <- gr.list[[x]]
  ...
  if(return_only_df){
    ...
    #Plot matrix
    g<- ggplot()+
      geom_raster(data=mat[mat$strand == "pos",], aes(x=position, y=region, fill=signal),alpha=alpha_value)+
      geom_raster(data=mat[mat$strand == "neg",], aes(x=position, y=region, fill=-signal), alpha=alpha_value)+
      ggtitle(title)+
      scale_x_continuous("Position (bp)", expand = expand_scale(mult = c(0, 0)))+
      scale_y_continuous("Regions", expand = expand_scale(mult = c(0, 0)))+
      scale_fill_gradient2(low=neg_color, mid="white", high=pos_color)+
      theme_classic()+
      facet_wrap(~sample_name, scale= "free", ncol = ncol)+
      theme(panel.background = element_rect(fill = "transparent", colour = NA),  plot.background = element_rect(fill = "transparent", colour = NA),
            text=element_text(size= text_size), legend.position = "bottom", legend.key.size = unit(0.3, "cm"), panel.grid = element_blank(), panel.border = element_blank())
...
########################################################################################################################################################################################
plot_multiple_bw_nexus_heatmap<-function(regions.gr, sample.list, title="Heatmap of ChIP-nexus Signals", order="sum", output_file_name=NA, reduce=F, upstream=50, downstream=50,
                                         removal_threshold=.5, normalize_threshold=0.99, reverse=F, return_only_df = F, ncol = 2, alpha_value = 1, neg_color ="red",
                                         pos_color = "royalblue"){
  mat <- mclapply(names(sample.list), function(x){
  ...
  if(return_only_df){
    ...
    g<- ggplot()+
      ...
      theme(panel.background = element_rect(fill = "transparent", colour = NA),  
            ...
            strip.text.x = element_text(size = 8), # facet_wrap group name font size
...
#########################################################################################################################################################################
# Function to make multiple heatmap for standard_metapeak_matrix
#Required inputs:
# gr.list = a list of GRanges of regions desired to plot across
# file path to the normal non stranded bigwig file
...
plot_multiple_gr_standard_heatmap<-function(gr.list, sample, title="Heatmap of ChIP-seq Signals", order="sum", output_file_name=NA, reduce=F, upstream=50, downstream=50,alpha_value=1, 
                                            removal_threshold=.5, normalize_threshold=0.99, reverse=F, return_only_df = F, ncol = 2, text_size = 14, low_color= "white", high_color= "#67000d"){
  ...
  mat <- mclapply(names(gr.list), function(x){
    ...
    mat <- standard_metapeak_matrix(gr, sample, upstream = upstream,
                 downstream = downstream)
  ...
  if(return_only_df){
    ...
    #Plot matrix
    g <- ggplot()+
      geom_raster(data=mat, aes(x=position, y=region, fill=signal), alpha = alpha_value)+
      ggtitle(title)+
      scale_x_continuous("Position (bp)", expand = expand_scale(mult = c(0, 0)))+
      scale_y_continuous("Regions", expand = expand_scale(mult = c(0, 0)))+
      scale_fill_gradient(low=low_color, high= high_color)+
...
# Function to make multiple heatmap for standard_metapeak_matrix
#Required inputs:
# bw.list = a list of file pathes for bigwig files
# file path to the grange object

plot_multiple_bw_standard_heatmap<-function(gr, bw.list, title="Heatmap of ChIP-seq Signals", order="sum", output_file_name=NA, reduce=F, upstream=50, downstream=50, alpha_value=1, 
                                            removal_threshold=.5, normalize_threshold=0.99, reverse=F, return_only_df = F, ncol = 2, low_color= "white", high_color= "#67000d"){

  #Get signals across regions
  mat <- mclapply(names(bw.list), function(x){
    bw <- bw.list[[x]]
    ...
    mat <- standard_metapeak_matrix(gr, bw, upstream = upstream,
                 downstream = downstream)
...
plot_sequence<-function(gr, title, genome, subset=F, cluster=F, show=T, window=201, y_axis_name = "Regions", x_axis_name = "Motif distance (bp)"){
  ...
  if(cluster){
    dna_dist <- DistanceMatrix(getSeq(genome, resized_to_desired_window), type="dist") # returns an object of class 'dist'
    dna_clust<-hclust(d = dna_dist)
    dna_order<-dna_clust$order
    sequence.df$row<-factor(sequence.df$row, levels=dna_order)
  }
  else{sequence.df$row<-factor(sequence.df$row)}
  ...
  #Plot
  sequence.df$row<-sequence.df$row %>% as.character %>% as.integer
  sequence.df$position<-sequence.df$position %>% as.character %>% as.integer

  g<-ggplot(sequence.df, aes(x=position, y=row, fill=nt))+
    geom_tile()+
...
plot_nexus_metapeak<-function(regions.gr, sample, title="Metapeak of ChIP-nexus Signals", output_file_name=NA, upstream=50, downstream=50, return_only_df = F){
