...
def bpnet_export_large_bw(model_dir,
    ...
    #
    # #Merge bigwigs by chromosome, convert bdg to BW, and clean up environment using Linux commands.
    # logger.info(f"Merging bw files to bedGraph...")
    # for task in bp.tasks:
    #     #Note: for UCSC commands, `subprocess.call` needs to be a single string, otherwise will return an output without running command.
    #     subprocess.call([f'bigWigMerge {output_prefix}_*.{task}.preds.pos.bw {output_prefix}.{task}.preds.pos.bedGraph'], shell=True)
    #     subprocess.call([f'bigWigMerge {output_prefix}_*.{task}.preds.neg.bw {output_prefix}.{task}.preds.neg.bedGraph'], shell=True)
    #     if 'counts/pre-act' in contrib_wildcard:
    #         subprocess.call([f'bigWigMerge {output_prefix}_*.{task}.contrib.counts.bw {output_prefix}.{task}.contrib.counts.bedGraph'], shell=True)
    #     if 'profile/wn' in contrib_wildcard:
    #         subprocess.call([f'bigWigMerge {output_prefix}_*.{task}.contrib.profile.bw {output_prefix}.{task}.contrib.profile.bedGraph'], shell=True)
    # logger.info(f"Sort bedGraph as case-sensitive...")
    # for task in bp.tasks:
    #     subprocess.call([f'sort -k1,1 -k2,2n {output_prefix}.{task}.preds.pos.bedGraph > {output_prefix}.{task}.preds.pos.sorted.bedGraph'], shell=True)
    #     subprocess.call([f'sort -k1,1 -k2,2n {output_prefix}.{task}.preds.neg.bedGraph > {output_prefix}.{task}.preds.neg.sorted.bedGraph'], shell=True)
    #     if 'counts/pre-act' in contrib_wildcard:
    #         subprocess.call([f'sort -k1,1 -k2,2n {output_prefix}.{task}.contrib.counts.bedGraph > {output_prefix}.{task}.contrib.counts.sorted.bedGraph'], shell=True)
    #     if 'profile/wn' in contrib_wildcard:
    #         subprocess.call([f'sort -k1,1 -k2,2n {output_prefix}.{task}.contrib.profile.bedGraph > {output_prefix}.{task}.contrib.profile.sorted.bedGraph'], shell=True)
    #
    # logger.info(f"Converting to bedGraph to bigWig...")
    # for task in bp.tasks:
    #     subprocess.call([f'bedGraphToBigWig {output_prefix}.{task}.preds.pos.sorted.bedGraph {chrom_sizes} {output_prefix}.{task}.preds.pos.bw'], shell=True)
    #     subprocess.call([f'bedGraphToBigWig {output_prefix}.{task}.preds.neg.sorted.bedGraph {chrom_sizes} {output_prefix}.{task}.preds.neg.bw'], shell=True)
    #     if 'counts/pre-act' in contrib_wildcard:
    #         subprocess.call([f'bedGraphToBigWig {output_prefix}.{task}.contrib.counts.sorted.bedGraph {chrom_sizes} {output_prefix}.{task}.contrib.counts.bw'], shell=True)
    #     if 'profile/wn' in contrib_wildcard:
    #         subprocess.call([f'bedGraphToBigWig {output_prefix}.{task}.contrib.profile.sorted.bedGraph {chrom_sizes} {output_prefix}.{task}.contrib.profile.bw'], shell=True)
    #
    # logger.info(f"Cleaning intermediate files...")
    # subprocess.call([f'rm {output_prefix}*.bedGraph'], shell=True)
    # subprocess.call([f'rm {output_prefix}*_chr*.bw'], shell=True)
    #
